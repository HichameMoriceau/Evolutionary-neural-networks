...
bool Population::epoch(int generation) {
  ...
  */
  ...
  oFile<<std::setprecision(7)<<std::fixed
       ...
       <<mean_fitness<<" " // mean
       <<"-1"<<" " // median
       <<organisms.size()<<" " // pop size
       <<(*(*(((*curspecies)->organisms).begin()))->net).inputs.size()<<" " // nb inputs

       <<(*(*(((*curspecies)->organisms).begin()))->net).numnodes<<" " // nb hidden units
       <<(*(*(((*curspecies)->organisms).begin()))->net).outputs.size()<<" " // nb outputs
       <<"-1"<<" "  // nb hidden layers
       <<"1"<<" " // true
       <<"-1"<<" " // mutation scheme
    
       <<"-1"<<" " // ann ensemble accuracy
...
double Population::mean(std::vector<double> array){
  double sum=0;
...
double Population::var(std::vector<double> array){
  double m=mean(array);
  double tmp=0;
  unsigned int size=array.size();
  for(unsigned int i=0; i<size; i++)
    tmp+=(m-array[i])*(m-array[i]);
  return tmp/size;
}

double Population::stddev(std::vector<double> array){
