...
hold on;
grid on;
plot(results(:,1),results(:,16), 'm', 'LineWidth', 1);
plot(results(:,1),results(:,18), 'g', 'LineWidth', 1, 'linestyle', '--');
...
legend('[Error amongst replicates] Corrected sample standard deviation','Prediction accuracy','Best indiv score on training set', 'Ensemble accuracy', 'Best indiv accuracy on CV set', "location", "southeast");
...
plot(results(:,20),results(:,16), 'm');
hold on;
grid on;
plot(results(:,20),results(:,16), 'm', 'LineWidth', 1);
plot(results(:,20),results(:,3),  'r', 'LineWidth', 1);
plot(results(:,20),results(:,4),  'k', 'LineWidth', 1);
plot(results(:,20),results(:,18), 'g', 'LineWidth', 1, 'linestyle', '--');
title( strcat(data_set_name, " - population size : ", num2str(population_size), " - ", num2str(nb_replicates), " replicates"));
legend('[Error amongst replicates] Corrected sample standard deviation', 'Ensemble accuracy', 'Best indiv accuracy on training set','Best indiv f1 Score on training set', 'Best indiv accuracy on CV set', "location", "southeast");
xlabel('Number of calls to the error function');
ylabel('Performance of best individual, ensemble set');
text(align_right, 50 , strcat("Score on unseen test data =", num2str(TEST_SCORE)));
text(align_right, 46 , strcat("Score on unseen test data =", num2str(TEST_ACC)));
# add topology description
text(align_right, 42 , "Final topology:");
